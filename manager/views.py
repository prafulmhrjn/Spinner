...
def addRooms(request , slot_id = None):
    ...
    return render(request , 'manager/addField.html' , content)
...
def addRooms_2(request , slot_id = None):
    ...
    if request.POST: 
        ...
        if get_room_count_form.is_valid():
            ...
            for room_no in range(1 ,n.fields_available+1):
                r = Field(chosen_date_slot = selected_slot_obj , room_number = room_no )
                r.save()
    ...
    return render(request , 'manager/addField2.html' , content)
...
def getAllBookedDates(request):
    ...
    # room_prices = DateAndSlot.objects.filter(rooms_added = True).order_by('room_price')
    content = {
...
def getSlotBookingDetail(request , slot_id):
    ...
    # bookings_per_price = RoomBooking.objects.filter(date_price = slot_id).order_by('date_price')
    content = {
            'bookings_per_date' : bookings_per_date
            # 'bookings_per_price' : bookings_per_price
    }
